...
#include"command_handler.h"
extern gas_api* p_gas;//load_library();
    ...
    if (strcmp(splitArray[1], "--help"))
        if ((sizeof(splitArray) / sizeof(char*)) > 2)
        {
            ...
            return;
             ...
             h = ht_search(ht, splitArray[0]).help;
            if (h)
                printf("%s", h);
            else
            {
...
{
    void* libHandle=dlopen("/home/bitya/build-try_dll-Desktop-Debug/libtry_dll.so.1.0.0",RTLD_LAZY);
    assert(libHandle);
    gas_api* p_gas;
